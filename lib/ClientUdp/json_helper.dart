...
class JsonHelper {
   ...
   */
  static Map<String, dynamic> createRequest(String action, Map<String, dynamic>? data) {
   ...
   */
  static DecryptedResult? decryptAndParse(
      Datagram packet, String keyString, {bool logTrace = false}) {
    if (packet.data.isEmpty) {
      logger.log('Received empty packet for decryption.');
      return null;
    }
    if (keyString.isEmpty) {
      logger.log('Attempted decryption with empty key from ${packet.address.address}:${packet.port}');
      return null;
    }
    ...
    try {
      ...
      if (logTrace) {
        logger.log('Received raw encrypted string: $encryptedString');
      }

      // Decrypt using Caesar cipher
      String decryptedJsonString = CaesarCipher.decrypt(encryptedString, keyString);
      if (decryptedJsonString.isEmpty) {
        logger.log('Decryption returned empty string for packet from ${packet.address.address}:${packet.port} with key length ${keyString.length}');
        return null; // Decryption failed critically
      }

      if (logTrace) {
        logger.log('Decrypted JSON string: $decryptedJsonString');
      }
      ...
      if (logTrace) {
        try {
          String partialDecryption = CaesarCipher.decrypt(utf8.decode(packet.data, allowMalformed: true), keyString);
          logger.log('Invalid JSON syntax after decryption with key length ${keyString.length} from ${packet.address.address}:${packet.port}. '
  ...
  static String _fixCommonDecryptionIssues(String decryptedText) {
    ...
    logger.log('Original decrypted text before fix: $decryptedText');
    ...
    fixed = fixed.replaceAll(RegExp(r'(?<=[\{\]\w"])\s*E\s*(?=[\{\[\w"])'), ',');
   ...
   */
  static Map<String, dynamic> createReply(String action, String status, String? message, Map<String, dynamic>? data) {
   ...
   */
  static Map<String, dynamic> createErrorReply(String originalAction, String errorMessage) {
    return {
   ...
   */
  static bool sendPacket(RawDatagramSocket socket, InternetAddress address, int port, 
      Map<String, dynamic> jsonData, String encryptionKey) {
    if (socket == null || address == null || jsonData == null) {
      logger.log('Attempted to send packet with null socket, address, or JSON data.');
      return false;
    }
    if (encryptionKey.isEmpty) {
      logger.log('Attempted encryption with empty key for packet to ${address.address}:$port');
      return false;
    }

    try {
      // Convert Dart Map to JSON string - this is equivalent to gson.toJson() in Java
      String jsonString = json.encode(jsonData); // This is serialization, not encryption
      ...
      logger.log('Encrypted with key: "$encryptionKey", length: ${encryptionKey.length}');
      
      // Log detailed encryption info for debugging
      logger.log('Sending encrypted text: $encryptedText');
      print('--------------------------------------Encrypted JSON string to send: $encryptedText'); // For debugging
      ...
      if (data.length > Constants.MAX_UDP_PACKET_SIZE) {
        logger.log('Attempted to send UDP packet larger than max size (${data.length} bytes) after encryption to ${address.address}:$port');
        return false;
      }
      ...
      String action = jsonData.containsKey(Constants.KEY_ACTION) 
          ...
          : "unknown";
      logger.log('Sent encrypted packet (action: $action) with key length ${encryptionKey.length} to ${address.address}:$port');
      
      return true;
    } catch (e, stackTrace) {
      logger.log('Error sending encrypted UDP packet to ${address.address}:$port: $e');
